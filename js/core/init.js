...
console.log(this.width_screen)
    ...
    StaticEnvelopeScenario(SYSTEM_DELTA_T, SAMPLES) {
        ...
        var width = 2 * (this.width_screen - this.width_screen_offset);
        var width_rate = width / (SYSTEM_DELTA_T * SAMPLES);
    ...
    StepEnvelope_template() {
        ...
        var width = 2*(this.width_screen - this.width_screen_offset);
        var width_second = (width / this.SYSTEM_SIMULATION_TIME)
        var second_knob = this.limit_values[this.selected_knob]

        //        var width = 1.82 * (this.width_screen - this.width_screen_offset);
        var expander = this.SYSTEM_SIMULATION_TIME / this.limit_values[this.selected_knob]
        ...
        if (this.is_template_loaded == false) {
            ...
            $(envelope_visualizer).append($bar);
            var $bar = $(document.createElementNS("http://www.w3.org/2000/svg", "rect")).attr({
                x: this.width_screen_offset + 5 * width_second,
                y: 50,
                width: 1,
                height: 240,
                stroke: "blue"
            ...
            var $bar = $(document.createElementNS("http://www.w3.org/2000/svg", "rect")).attr({
                x: this.width_screen_offset + 5 * width_second,
                y: 50,
                width: this.width_screen_offset + 15 * width_second,
                height: 1,
                stroke: "green"
            });
            this.is_template_loaded = true;
            ...
            var $bar = $(document.createElementNS("http://www.w3.org/2000/svg", "rect")).attr({
                x: curr_width,
                y: 290,
                width: (knob[0] / this.SYSTEM_SIMULATION_TIME) * width_rate,
                height: 1,
                stroke: "red"
            });
            $(envelope_visualizer).append($bar);
            curr_width += ((knob)[0] / this.SYSTEM_SIMULATION_TIME) * width_rate
            var i = 5;
            while (in_time == true) {
                var $bar = $(document.createElementNS("http://www.w3.org/2000/svg", "rect")).attr({
                    x: curr_width,
                ...
                var $bar = $(document.createElementNS("http://www.w3.org/2000/svg", "rect")).attr({
                    x: curr_width,
                    y: 150,
                    width: ((knob)[1] / this.SYSTEM_SIMULATION_TIME) * width_rate,
                    height: 1,
                    stroke: "green"
                });
                $(envelope_visualizer).append($bar);
                curr_width += ((knob)[1] / this.SYSTEM_SIMULATION_TIME) * width_rate
                ...
                var $bar = $(document.createElementNS("http://www.w3.org/2000/svg", "rect")).attr({
                    x: curr_width,
                    y: 290,
                    width: ((knob)[2] / this.SYSTEM_SIMULATION_TIME) * width_rate,
                    height: 1,
                    stroke: "magenta"
                });
                $(envelope_visualizer).append($bar);
                curr_width += ((knob)[2] / this.SYSTEM_SIMULATION_TIME) * width_rate
